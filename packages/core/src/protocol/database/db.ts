...
export class Db {
  ...
  static async openDb(publicKey: string, seed: string) {
    ...
    db.version(4).stores({
      messages:
        '++id, &message_id, &payload_id, sent_at, received_at, [is_outbound+is_delete], [target_id+target_type], [target_id+target_type+is_delete+sent_at], [target_id+target_type+is_outbound+is_delete], [payload_id+status]',
